...
namespace RazorpayFluentCart\PluginManager;
...
class Updater
{
    ...
    private function get_fallback_plugin_info()
    {
        ...
        $info->sections = [
            'description' => sprintf(
                ...
                esc_html__('Full version details are available on the plugin page.', 'razorpay-for-fluent-cart'),
                esc_url($plugin_page_url),
                esc_html__('View Plugin Page &rarr;', 'razorpay-for-fluent-cart')
            ),
        ];
    ...
    private function api_request()
    {
        ...
        if ($request && isset($request->sections)) {
            // Verify the response is for this addon, not the parent product.
            // If the store doesn't have the addon-assets plugin, it returns
    ...
    public function show_license_activation_notice()
    {
        ...
        $message = sprintf(
            __('%1$s updates require an active FluentCart Pro license. Please activate your FluentCart Pro license to receive updates.', 'razorpay-for-fluent-cart'),
        ...
        printf(
            ...
            esc_url($activateUrl),
            esc_html__('Activate License', 'razorpay-for-fluent-cart')
        );
    }
    ...
    private function maybe_delete_transients()
    {
        ...
        if (isset($_GET['razorpay-for-fluent-cart-check-update'])) {
            if (current_user_can('update_plugins')) {
                $this->delete_transients();
                ...
                wp_redirect(admin_url('plugins.php?s=razorpay-for-fluent-cart&plugin_status=all'));
                exit();
            }
        }
    }
